    ...
    _doApplyEdits(rawOperations, computeUndoEdits) {
        ...
        if (contentChanges.length !== 0) {
            ...
            for (let i = 0, len = contentChanges.length; i < len; i++) {
                ...
                const decorationsWithInjectedTextInEditedRange = this._ensureNodesHaveRanges(this._decorationsTree.getInjectedTextInInterval(this.getOffsetAt(new Position(firstEditLineNumber, 1)), this.getOffsetAt(new Position(lastInsertedLineNumber, this.getLineMaxColumn(lastInsertedLineNumber))), 0, this._versionId));
...
class DecorationsTrees {
    ...
    getInjectedTextInInterval(start, end, filterOwnerId, cachedVersionId) {
        return this._injectedTextDecorationsTree.intervalSearch(start, end, filterOwnerId, false, cachedVersionId);
    }
    getInjectedText(filterOwnerId, cachedVersionId) {
